        ...
        while sum([tup[3] for tup in self.k_best]) < self.k:
            ...
            for seq, env, score, done, _, indices in self.k_best:
                if not done:
                    ...
                    out = torch.rand(17).unsqueeze(0)#self.policy(state) 
                    out = out * mask
                    ...
                    for log_prob, action in zip(log_probs, actions):
                        ...
                        candidates.append([seq + [action.item()], copy.deepcopy(env), score - log_prob.item(), indices, vehicle_id])
            ...
            for i, (actions, env, scores, indices, vehicle_id) in enumerate(ordered):
                action = actions[-1]
